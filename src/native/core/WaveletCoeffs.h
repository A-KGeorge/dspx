 ...
 *
 * Daubechies wavelet filter coefficients for discrete wavelet transforms.
 * Sourced from PyWavelets project.
 *
 * Each wavelet is defined by its scaling (low-pass decomposition) coefficients.
 * The high-pass decomposition filter is derived by quadrature mirror filter.
 */

#ifndef DSP_CORE_WAVELET_COEFFS_H
#define DSP_CORE_WAVELET_COEFFS_H
...
namespace dsp
{
    ...
    {
        /**
         * Wavelet filter banks (decomposition and reconstruction)
         */
        struct WaveletFilters
        {
            std::vector<double> dec_lo; // Decomposition low-pass (scaling)
            std::vector<double> dec_hi; // Decomposition high-pass (wavelet)
            std::vector<double> rec_lo; // Reconstruction low-pass
            std::vector<double> rec_hi; // Reconstruction high-pass
        };

        /**
         * Generate quadrature mirror filter (QMF) from scaling coefficients
         * For orthogonal wavelets: h[n] = (-1)^n * g[N-1-n]
         */
        inline std::vector<double> generateQMF(const std::vector<double> &scaling)
        {
            std::vector<double> wavelet(scaling.size());
            for (size_t i = 0; i < scaling.size(); ++i)
            {
                size_t reversed_idx = scaling.size() - 1 - i;
                wavelet[i] = ((i % 2) == 0 ? 1.0 : -1.0) * scaling[reversed_idx];
            }
            return wavelet;
        }

        /**
         * Generate reconstruction filters from decomposition filters
         * For orthogonal wavelets: reconstruction = time-reversed decomposition
         */
        inline WaveletFilters generateFilters(const std::vector<double> &scaling_coeffs)
        {
            WaveletFilters filters;
            filters.dec_lo = scaling_coeffs;
            filters.dec_hi = generateQMF(scaling_coeffs);

            // Reconstruction filters are time-reversed
            filters.rec_lo = std::vector<double>(scaling_coeffs.rbegin(), scaling_coeffs.rend());
            filters.rec_hi = std::vector<double>(filters.dec_hi.rbegin(), filters.dec_hi.rend());

            return filters;
        }

        // ========== Haar Wavelet (db1) ==========
        inline WaveletFilters getHaarFilters()
        {
            static const double INV_SQRT2 = 1.0 / std::sqrt(2.0);
            return generateFilters({INV_SQRT2, INV_SQRT2});
        }

        // ========== Daubechies 2 (db2) ==========
        inline WaveletFilters getDb2Filters()
        {
            static const double s3 = std::sqrt(3.0);
            static const double INV_4_SQRT2 = 1.0 / (4.0 * std::sqrt(2.0));
            return generateFilters({(1.0 + s3) * INV_4_SQRT2,
                                    (3.0 + s3) * INV_4_SQRT2,
                                    (3.0 - s3) * INV_4_SQRT2,
                                    (1.0 - s3) * INV_4_SQRT2});
        }
        ...
        // ========== Daubechies 3 (db3) ==========
        inline WaveletFilters getDb3Filters()
        {
            return generateFilters({3.326705529500826159985115891390056300129233992450683597084705e-01,
        ...
        // ========== Daubechies 4 (db4) ==========
        inline WaveletFilters getDb4Filters()
        {
            return generateFilters({2.303778133088965008632911830440708500016152482483092977910968e-01,
        ...
        // ========== Daubechies 5 (db5) ==========
        inline WaveletFilters getDb5Filters()
        {
            return generateFilters({1.601023979741929144807237480204207336505441246250578327725699e-01,
        ...
        // ========== Daubechies 6 (db6) ==========
        inline WaveletFilters getDb6Filters()
        {
            return generateFilters({1.115407433501094636213239172409234390425395919844216759082360e-01,
        ...
        // ========== Daubechies 7 (db7) ==========
        inline WaveletFilters getDb7Filters()
        {
            return generateFilters({7.785205408500917901996352195789374837918305292795568438702937e-02,
        ...
        // ========== Daubechies 8 (db8) ==========
        inline WaveletFilters getDb8Filters()
        {
            return generateFilters({5.441584224310400995500940520299935503599554294733050397729280e-02,
        ...
        // ========== Daubechies 9 (db9) ==========
        inline WaveletFilters getDb9Filters()
        {
            return generateFilters({3.807794736387834658869765887955118448771714496278417476647192e-02,
        ...
        // ========== Daubechies 10 (db10) ==========
        inline WaveletFilters getDb10Filters()
        {
            return generateFilters({2.667005790055555358661744877130858277192498290851289932779975e-02,
         ...
         */
        inline WaveletFilters getWaveletFilters(const std::string &name)
        {
            if (name == "haar" || name == "db1")
                return getHaarFilters();
            if (name == "db2")
                return getDb2Filters();
            if (name == "db3")
                return getDb3Filters();
            if (name == "db4")
                return getDb4Filters();
            if (name == "db5")
                return getDb5Filters();
            if (name == "db6")
                return getDb6Filters();
            if (name == "db7")
                return getDb7Filters();
            if (name == "db8")
                return getDb8Filters();
            if (name == "db9")
                return getDb9Filters();
            if (name == "db10")
                return getDb10Filters();
...
#endif // DSP_CORE_WAVELET_COEFFS_H
