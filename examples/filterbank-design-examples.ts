...
for (let i = 0; i < 5; i++) {
  console.log(
    `  Band ${i + 1}: ${melBoundaries[i].toFixed(1)} Hz - ${melBoundaries[
      i + 1
    ].toFixed(1)} Hz`
  );
  console.log(
    `    b coeffs (${melBank[i].b.length}): [${melBank[i].b
      .slice(0, 3)
      .map((c) => c.toFixed(6))
      .join(", ")}, ...]`
  );
  console.log(
    `    a coeffs (${melBank[i].a.length}): [${melBank[i].a
      .slice(0, 3)
      .map((c) => c.toFixed(6))
      .join(", ")}, ...]`
  );
...
for (let i = 0; i < octaveBoundaries.length - 1; i++) {
  ...
  console.log(
    `  Band ${i + 1}: ${octaveBoundaries[i].toFixed(1)} - ${octaveBoundaries[
      i + 1
    ].toFixed(1)} Hz (center: ${center.toFixed(1)} Hz)`
  );
}
...
const scales = ["linear", "log", "mel", "bark"] as const;
const count = 10;
const sampleRate = 16000;
const range: [number, number] = [100, 8000];
...
for (const scale of scales) {
  ...
  console.log(
    `  Bandwidth range: ${minBandwidth.toFixed(1)} - ${maxBandwidth.toFixed(
      1
    )} Hz`
  );
  console.log(
    `  First band: ${boundaries[0].toFixed(1)} - ${boundaries[1].toFixed(
      1
    )} Hz (${bandwidths[0].toFixed(1)} Hz)`
  );
  console.log(
    `  Last band: ${boundaries[count - 1].toFixed(1)} - ${boundaries[
      count
    ].toFixed(1)} Hz (${bandwidths[count - 1].toFixed(1)} Hz)`
  );
  console.log("");
}
